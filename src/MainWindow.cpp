    ...
    ui(new Ui::MainWindow)
{
    ...
    ui->label_mas_H->setText(QString::number(0)+"%");
    ui->mas_horizontalSlider_H->setRange(0,100);
    ui->mas_horizontalSlider_H->setSingleStep(5);

    ui->label_menos_H->setText(QString::number(0)+"%");
    ui->menos_horizontalSlider_H->setRange(0,100);
    ui->menos_horizontalSlider_H->setSingleStep(5);

    ui->label_mas_V->setText(QString::number(0)+"%");
    ui->mas_horizontalSlider_V->setRange(0,100);
    ui->mas_horizontalSlider_V->setSingleStep(5);

    ui->label_menos_V->setText(QString::number(0)+"%");
    ui->menos_horizontalSlider_V->setRange(0,100);
    ui->menos_horizontalSlider_V->setSingleStep(5);
    for (int i=1;i<=6;i++){
...
void MainWindow::setInitialConfigurations()
{
    ...
    ui->label_adq_tasa_config->setVisible(false);
    ui->label_adq_error_config->setVisible(false);
...
void MainWindow::receivedHitsTiempos(QVector<double> hits, int channels, QString pmt_head, int index, bool mode)
{
    addGraph_Tiempos(hits, ui->specPMTs_2, channels, pmt_head, qcp_pmt_calib_parameters[index]);
}
...
void MainWindow::on_pushButton_configure_clicked()
{
    ...
    try
    {
        ...
        case COIN_NORMAL:
            ...
            usleep(5000);
            setTimeModeCoin(COIN_NORMAL,false);
            break;
        case COIN_AUTOCOINCIDENCE:
            ...
            usleep(5000);
            setTimeModeCoin(COIN_AUTOCOINCIDENCE,false);
            break;
        case COIN_AVANCED:
            ...
            usleep(5000);
            setTimeModeCoin(COIN_AVANCED,false);
            break;
        case COIN_CALIB:
            ...
            usleep(5000);
            setTimeModeCoin(COIN_CALIB,false, head);
            break;
        case COIN_VERIF:
            head = ui->comboBox_head_select_calib->currentText();
            if(debug) cout<<"Modo Calibración en el cabezal: "<<head.toStdString()<<endl;
            ...
            setCalibrationMode(head);
            setTimeModeCoin(COIN_VERIF,false, head);
            break;
        case COIN_INTER_CABEZAL:
            ...
            setCoincidenceModeDataStream(arpet->getNormal_Coin_Mode());
            usleep(5000);
            setTimeModeCoin(COIN_INTER_CABEZAL,false);
            break;
        default:
            break;
        }
...
void MainWindow::setTimeModeCoin(int modo, bool cero, QString head){
        ...
        case COIN_NORMAL:
            for ( int i = 0; i < Estado_Cabezales.length(); i++ )
            {
                ...
                for(int pmt = 0; pmt < PMTs; pmt++){
                    ...
                    if(cero) // Manda solo Intracabezal
...
string MainWindow::initHead(int head)
{
    ...
    if (head<7) setLabelState(arpet->verifyMCAEStream(msg_head,arpet->getAnsHeadInit()),head_status_table[head-1]);
...
/**
 * @brief MainWindow::drawAlmohada
 *
 * Dibuja almohadón
 *
 */
void MainWindow::drawAlmohada()
{
    ...
    try
    {
        for (int i=0;i<checkedHeads.length();i++)
        {
            ...
            arpet->InitSP3(QString::number(head_index).toStdString(),port_name.toStdString());
            if(debug) cout<<"Pasa init mca sp3: "<<endl;
            string msg = arpet->getAlmohada(QString::number(head_index).toStdString(), port_name.toStdString());
            cout << msg << endl;
            arpet->portDisconnect();

            QVector<double> vec_aux = arpet->getHitsMCA();

            double max_vec = *std::max_element(vec_aux.begin(), vec_aux.end());

            QVector<float> vec;

            for (int j=0;j<32;j++)
                for (int k=0;k<32;k++){
                    if(vec_aux.at(j+k*32) > max_vec/32)
                        vec.append((qLn((vec_aux.at(j+k*32)))));
                    else
                       vec.append(0);
                }
            ...
            double co = (256*256*256-1)/((max));

            QImage tmpImage(32, 32, QImage::Format_RGB32);

            for (int j=0;j<32;j++)
                for (int k=0;k<32;k++){
                    tmpImage.setPixel(j,k,qCeil((vec.at(j+k*32)*co)));
                }

            switch (head_index) {
            case 1:
                ui->label_almohada_1->setVisible(true);
                ui->label_almohada_1->setPixmap(QPixmap::fromImage(tmpImage));
                ui->label_almohada_1->setScaledContents(true);
                ui->label_almohada_1->show();
                break;
            case 2:
                ui->label_almohada_2->setVisible(true);
                ui->label_almohada_2->setPixmap(QPixmap::fromImage(tmpImage));
                ui->label_almohada_2->setScaledContents(true);
                ui->label_almohada_2->show();
                break;
            case 3:
                ui->label_almohada_3->setVisible(true);
                ui->label_almohada_3->setPixmap(QPixmap::fromImage(tmpImage));
                ui->label_almohada_3->setScaledContents(true);
                ui->label_almohada_3->show();
                break;
            case 4:
                ui->label_almohada_4->setVisible(true);
                ui->label_almohada_4->setPixmap(QPixmap::fromImage(tmpImage));
                ui->label_almohada_4->setScaledContents(true);
                ui->label_almohada_4->show();
                break;
            case 5:
                ui->label_almohada_5->setVisible(true);
                ui->label_almohada_5->setPixmap(QPixmap::fromImage(tmpImage));
                ui->label_almohada_5->setScaledContents(true);
                ui->label_almohada_5->show();
                break;
            case 6:
                ui->label_almohada_6->setVisible(true);
                ui->label_almohada_6->setPixmap(QPixmap::fromImage(tmpImage));
                ui->label_almohada_6->setScaledContents(true);
                ui->label_almohada_6->show();
                break;
            default:
                break;
            }
    ...
    {
        if(debug) cout<<"Imposible obtener almohadón. Error: "<<ex.excdesc<<endl;
        QMessageBox::critical(this,tr("Atención"),tr((string("Imposible obtener los valores de temperatura. Revise la conexión al equipo. Error: ")+string(ex.excdesc)).c_str()));
        arpet->portDisconnect();
    }
...
void MainWindow::setAdquireMode(int index)
{
    ...
    switch (adquire_mode) {
        ...
        case ALMOHADA:
            ui->frame_PMT->hide();
            ui->frame_HV->hide();
            ui->tabWidget_mca->setCurrentWidget(ui->tab_esp_4);
            ui->frame_MCA->hide();
            ui->checkBox_espectro_calibrado->hide();
            ui->comboBox_head_select_graph->hide();
            ui->frame_multihead_graph->show();
...
bool MainWindow::resetPMTs()
{
    ...
    try
    {
        setHV(QString::number(checkedHeads.at(0)).toStdString(), QString::number(LowLimit[checkedHeads.at(0)]).toStdString());
    }
    catch( Exceptions & ex )
    {
        status = false;
...
void MainWindow::on_pushButton_hv_configure_clicked()
{
    ...
    try
    {
        ...
        if (ui->checkBox_centroid->isChecked()) setHV(head.toStdString(), QString::number(LowLimit[head.toInt()-1]).toStdString());
        q_msg =setHV(head.toStdString(),getHVValue(ui->lineEdit_hv_value),pmt_selected_list.at(0).toStdString());
...
void MainWindow::on_pushButton_l_5_clicked()
{
    ...
    try
    {
        ...
        getMCA(head.toStdString(),arpet->getFunCSP3(),true,CHANNELS_PMT,pmt_selected_list.at(0).toStdString());
        ui->lineEdit_hv_value->setText(QString::number(arpet->getHVMCA()));
        if (ui->checkBox_centroid->isChecked()) setHV(head.toStdString(), QString::number(LowLimit[head.toInt()-1]).toStdString());
        q_msg = setHV(head.toStdString(),getHVValue(ui->lineEdit_hv_value,-5),pmt_selected_list.at(0).toStdString());
        getMCA(head.toStdString(),arpet->getFunCSP3(),true,CHANNELS_PMT,pmt_selected_list.at(0).toStdString());
...
void MainWindow::on_pushButton_l_10_clicked()
{
    ...
    try
    {
        ...
        ui->lineEdit_hv_value->setText(QString::number(arpet->getHVMCA()));
        if (ui->checkBox_centroid->isChecked()) setHV(head.toStdString(), QString::number(LowLimit[head.toInt()-1]).toStdString());
        q_msg = setHV(head.toStdString(),getHVValue(ui->lineEdit_hv_value,-10),pmt_selected_list.at(0).toStdString());
        getMCA(head.toStdString(),arpet->getFunCSP3(),true,CHANNELS_PMT,pmt_selected_list.at(0).toStdString());
        if(debug) cout<<"El nuevo canal configurado en el PMT "<<pmt_selected_list.at(0).toStdString()<<" es: "<<ui->lineEdit_hv_value->text().toStdString()<<endl;
    }
    catch (Exceptions ex)
...
void MainWindow::on_pushButton_l_50_clicked()
{
    ...
    try
    {
        ...
        ui->lineEdit_hv_value->setText(QString::number(arpet->getHVMCA()));
        if (ui->checkBox_centroid->isChecked()) setHV(head.toStdString(),QString::number(LowLimit[head.toInt()-1]).toStdString());
        q_msg = setHV(head.toStdString(),getHVValue(ui->lineEdit_hv_value,-50),pmt_selected_list.at(0).toStdString());
        getMCA(head.toStdString(),arpet->getFunCSP3(),true,CHANNELS_PMT,pmt_selected_list.at(0).toStdString());
        if(debug) cout<<"El nuevo canal configurado en el PMT "<<pmt_selected_list.at(0).toStdString()<<" es: "<<ui->lineEdit_hv_value->text().toStdString()<<endl;
    }
    catch (Exceptions ex)
...
void MainWindow::setButtonCalibTiemposState(bool state, bool disable)
{
    ...
    {
        qt_text="Adquiriendo";
        setButtonState(state,ui->pb_Autocalib_Tiempos,disable);
    }
    else if (!state && !disable)
    {
        qt_text="Error";
    ...
    {
        qt_text="Calibrar Tiempos";
        setButtonState(state,ui->pb_Autocalib_Tiempos,disable);
    }
    ui->pb_Autocalib_Tiempos->setText(qt_text);
    ui->pb_Autocalib_Tiempos->update();
}
 ...
 * @param show
 */
void MainWindow::manageHeadComboBox(string tab)
{
    if(tab=="mca")
    {
        ui->comboBox_head_select_graph->hide();
...
void MainWindow::addGraph_Tiempos(QVector<double> hits,  QCustomPlot *graph, int channels, QString graph_legend, QVector<int> param,bool local)
{
    channels_ui.resize(channels);
    if(local)
        channels_ui = arpet->getChannels();
    ...
    for(int i=0;i<channels;i++)
        channels_aux.append(channels_ui.at(i)-128);
...
void MainWindow::on_pb_Autocalib_Tiempos_clicked()
{
    ...
    // Este MCa se va a hacer a las 9 máquinas de coincidencias en modo normal
    // Para esto primero tengo que configurar los cabezales en modo normal con tablas intercabezal en cero

    // Esto lo hago con la rutina de inicialización que usa Dami cuando va a adquirir (modificando lo de la tabla de tiempos)

    // Si todo sale bien espero el tiempo de acumulación (que habría que ver de cuánto lo hago), podría ser fijo o que espere a
    // que haya un mínimo total de cuentas en todas las máquinas (las más complicadas van a ser la de cabezales cruzados, pero hay que ver cómo se comportan)

    // Con ese MCA se calcula posición de pico, se sacan las relaciones, se configuran y se vuelve a empezar. Se podría usar la calibración que hizo Rama
...
void MainWindow::on_pb_Autocalib_Tiempos_Reset_clicked()
{
    error_code error_code;
    ...
    setHV(QString::number(7).toStdString(),QString::number(0).toStdString());

    clearSpecCalibGraphs();


//    resetHead(QString::number(7));
//    resetHitsValues(QString::number(7));
//    setPMTCustomPlotEnvironment(pmt_selected_list);
//    resetPMTs();//ui->checkBox_centroid->isChecked());
//    removeAllGraphsPMT();
}

void MainWindow::on_pb_Autocalib_Tiempos_Debug_clicked()
{
    error_code error_code;
 ...
 * @param checked
 */
void MainWindow::on_checkBox_MLEM_clicked()
{
 ...
 * @param checked
 */
void MainWindow::on_checkBox_Backprojection_clicked()
{
...
void MainWindow::updateCaption(){
    ...
    if(adq_running){
        if(calibrador->AutoCalibBackground){
            ...
            error=output.mid(output.indexOf("con") + 4);
...
void MainWindow::on_pbAdquirir_toggled(bool checked)
{
  ...
  //  Cabezales_On_Off(checked);
    ...
    killall.start("ps -C recvRawEth");
    QString output;
    while(killall.waitForReadyRead(1000)) output+=killall.readAll();

    if(output.contains("recvRawEth"))
    {
    ...
    if (checked){
        ...
        if(ui->comboBox_aqd_mode->currentIndex()==1){
            ...
            usleep(5000);
            setTimeModeCoin(COIN_CALIB,false, ui->cb_Calib_Cab->currentText());
        }else{
            if(debug) cout<<"Modo Coincidencia: Normal"<<endl;
            initCoincidenceMode();
            usleep(5000);
...

{
    ...
    QString filename = QFileDialog::getOpenFileName(this,
        tr("Abrir Archivo.bit"), QDir::homePath() , tr("Image Files (*.bit)"));
        ...
        image.load(icon_notok);
         ...
         if(calibrador->AutoCalibBackground)
         {
            ...
            CopyAdqReady(false);
             ...
             ui->label_adq_tasa_config->setVisible(false);
             ui->label_adq_error_config->setVisible(false);
             ui->progressBar_Adq_Config->setValue(0);
             Adq_zone=adq_none;
         }
         else
         {
...
void MainWindow::on_pbAdquirir_Config_toggled(bool checked)
{
    ...
    ui->label_gif_config_ok_no->hide();
    ui->label_adq_tasa_config->setVisible(true);
    ui->label_adq_error_config->setVisible(true);
...
bool MainWindow::sendCamilla(string command, QString port_name)
{
    try
    {
...
void MainWindow::on_pb_stop_camilla_clicked()
{
    string command = "#CARCASA02@STOP,";
    port_name=QString::fromUtf8(DEV_PATH)+QString::fromUtf8(CARCASA);
    if(sendCamilla(command,port_name))
    {
        ui->pb_send_giro->setChecked(false);
        ui->pb_send_mas_H->setChecked(false);
        ui->pb_send_menos_H->setChecked(false);
        ui->pb_send_mas_V->setChecked(false);
        ui->pb_send_menos_V->setChecked(false);
    }
}
...
void MainWindow::on_pb_send_giro_toggled(bool checked)
{
    ...
    {
        string command = "#CARCASA02@GIRO,1,";
        port_name=QString::fromUtf8(DEV_PATH)+QString::fromUtf8(CARCASA);
        if(sendCamilla(command,port_name))
        {
...
/*** HORIZONTAL ***/
void MainWindow::on_pb_send_mas_H_toggled(bool checked)
{
    if (checked)
    {
        ui->mas_horizontalSlider_H->setEnabled(true);
    }
    else
    {
        string command = "#CARCASA02@HORIZONTAL,1,";
        port_name=QString::fromUtf8(DEV_PATH)+QString::fromUtf8(CARCASA);
        if(sendCamilla(command,port_name))
        {
            ui->mas_horizontalSlider_H->setValue(0);
            ui->mas_horizontalSlider_H->setEnabled(false);
        }
    }
}

void MainWindow::on_mas_horizontalSlider_H_valueChanged(int value)
{
    string command;
    if(value > 0)
        command = "#CARCASA02@HORIZONTAL,0,"+QString::number(value).toStdString()+"0,0,0,0,";
    else
        command = "#CARCASA02@HORIZONTAL,1,";
    port_name=QString::fromUtf8(DEV_PATH)+QString::fromUtf8(CARCASA);
    if(sendCamilla(command,port_name))
    {
        ui->label_mas_H->setText(QString::number(value)+"%");
    }
}

void MainWindow::on_pb_send_menos_H_toggled(bool checked)
{
    if (checked)
    {
        ui->menos_horizontalSlider_H->setEnabled(true);
    }
    else
    {
        string command = "#CARCASA02@HORIZONTAL,1,";
        ...
        if(sendCamilla(command,port_name))
        {
            ui->menos_horizontalSlider_H->setValue(0);
            ui->menos_horizontalSlider_H->setEnabled(false);
        }
    }
}

void MainWindow::on_menos_horizontalSlider_H_valueChanged(int value)
{
    string command;
    if(value > 0)
    ...
    if(sendCamilla(command,port_name))
    {
        ui->label_menos_H->setText(QString::number(value)+"%");
    }
}

/*** VERTICAL ***/
void MainWindow::on_pb_send_mas_V_toggled(bool checked)
{
    if (checked)
    {
        ui->mas_horizontalSlider_V->setEnabled(true);
    }
    else
    {
        string command = "#CARCASA02@VERTICAL,1,";
        port_name=QString::fromUtf8(DEV_PATH)+QString::fromUtf8(CARCASA);
        if(sendCamilla(command,port_name))
        {
            ui->mas_horizontalSlider_V->setValue(0);
            ui->mas_horizontalSlider_V->setEnabled(false);
        }
    }
}

void MainWindow::on_mas_horizontalSlider_V_valueChanged(int value)
{
    string command;
    if(value > 0)
        command = "#CARCASA02@VERTICAL,0,"+QString::number(value).toStdString()+"0,0,0,0,";
    else
        command = "#CARCASA02@VERTICAL,1,";
    port_name=QString::fromUtf8(DEV_PATH)+QString::fromUtf8(CARCASA);
    if(sendCamilla(command,port_name))
    {
        ui->label_mas_V->setText(QString::number(value)+"%");
    }
}

void MainWindow::on_pb_send_menos_V_toggled(bool checked)
{
    if (checked)
    {
        ui->menos_horizontalSlider_V->setEnabled(true);
    }
    else
    {
        string command = "#CARCASA02@VERTICAL,1,";
        ...
        if(sendCamilla(command,port_name))
        {
            ui->menos_horizontalSlider_V->setValue(0);
            ui->menos_horizontalSlider_V->setEnabled(false);
        }
    }
}

void MainWindow::on_menos_horizontalSlider_V_valueChanged(int value)
{
    string command;
    if(value > 0)
        command = "#CARCASA02@VERTICAL,0,0,"+QString::number(value).toStdString()+"0,0,0,";
    else
        command = "#CARCASA02@VERTICAL,1,";
    port_name=QString::fromUtf8(DEV_PATH)+QString::fromUtf8(CARCASA);
    if(sendCamilla(command,port_name))
    {
        ui->label_menos_V->setText(QString::number(value)+"%");
    }
}

void MainWindow::on_pb_send_mas_H_2_pressed()
{
    if(!is_moving)
    {
        string command = "#CARCASA02@HORIZONTAL,0,700,0,0,0,";
        port_name=QString::fromUtf8(DEV_PATH)+QString::fromUtf8(CARCASA);
        if(sendCamilla(command,port_name))
            is_moving = true;
    }
...
void MainWindow::on_pb_send_mas_H_2_released()
{
    if(is_moving)
    {
        string command = "#CARCASA02@HORIZONTAL,1,";
...
void MainWindow::on_pb_send_menos_H_2_pressed()
{
    if(!is_moving)
    {
        string command = "#CARCASA02@HORIZONTAL,0,0,700,0,0,";
        port_name=QString::fromUtf8(DEV_PATH)+QString::fromUtf8(CARCASA);
        if(sendCamilla(command,port_name))
            is_moving = true;
    }
...
void MainWindow::on_pb_send_menos_H_2_released()
{
    if(is_moving)
    {
        string command = "#CARCASA02@HORIZONTAL,1,";
...
void MainWindow::on_pb_send_mas_V_2_pressed()
{
    if(!is_moving)
    {
...
void MainWindow::on_pb_send_mas_V_2_released()
{
    if(is_moving)
    {
        string command = "#CARCASA02@VERTICAL,1,";
...
void MainWindow::on_pb_send_menos_V_2_pressed()
{
    if(!is_moving)
    {
...
void MainWindow::on_pb_send_menos_V_2_released()
{
    if(is_moving)
    {
        string command = "#CARCASA02@VERTICAL,1,";
